...
Wrkr emits three proof record types:
...
- `approval` — an agent identity was approved, renewed, or revoked (approver, scope, expiry, reason)
...
- **MCP server supply chain analysis:** For each discovered MCP server, extract package origin (npm/PyPI/Go module), determine whether the package version is pinned or floating (`npx -y` with no lockfile = unpinned), check for known vulnerabilities via advisory databases, and verify whether the server is listed in the official MCP Registry (`registry.modelcontextprotocol.io`). Unpinned MCP packages are the exact attack vector used in real-world supply chain compromises (malicious Postmark MCP on npm).
- Support scanning: GitHub orgs (required), GitLab groups (v1.1), Bitbucket projects (v1.2)
...
- **MCP server supply chain trust:** Each MCP server receives a supply chain trust score (0-10) based on: package pinning (unpinned `npx -y` = 0, pinned with lockfile = high), known vulnerabilities (advisory count), MCP Registry verification (listed = trust boost, unlisted = unknown), transport type (stdio = local-only, HTTP = remote exposure). Servers with trust score below threshold (default: 3.0, configurable) are flagged as high-risk findings. This directly addresses the real-world attack vector: malicious MCP server packages published to npm that exfiltrate data through every agent that installs them.
...
- Generate evidence bundles mapped to: EU AI Act (Articles 9, 12, 14), SOC 2 (CC6, CC7), Texas TRAIGA, Colorado AI Act, ISO 42001, NIST AI 600-1, SOX, PCI-DSS
...
- **Scheduled mode (e.g., weekly):** Full org scan → posture delta since last scan → auto-open remediation PRs (FR6) → post posture score to Slack/webhook. This is the primary engagement loop — Wrkr produces work and a weekly narrative without anyone running a command.
...
- `wrkr init` — interactive setup (GitHub token, org, scan targets)
- `wrkr scan` — full scan, produces inventory + risk report + proof records
- `wrkr scan --diff` — incremental, shows changes since last scan
...
- **Lifecycle transitions:**
  ...
  - `deprecated`: Tool is flagged for removal. Emits `scan_finding` with deprecation reason. Grace period configurable.
  - `revoked`: Tool is explicitly blocked. Emits `scan_finding` with revocation reason. Wrkr regression (`wrkr regress run`) fails if a revoked tool reappears.
...
- Deterministic output: same input always produces same inventory and same proof records (no LLM in the scan pipeline — this is pattern matching and static analysis, not generative AI)
...
- Wrkr itself requests minimum permissions: read-only repo access, read-only org metadata
- GitHub token scopes required: `repo` (classic) or `contents: read` + `metadata: read` (fine-grained), `read:org`, `admin:org_hook` (optional, for App installs)
...
- Detector interface: `Detect(repoPath string) ([]Finding, error)`
...
5. **Not a SaaS product (v1).** The open-source CLI is the product. Wrkr Pro (dashboard, scheduling, Slack, SIEM) is the commercial extension, built later.
6. **Not cross-platform on day one.** GitHub is the only supported source in v1. GitLab and Bitbucket follow in v1.1 and v1.2.
...
- See a ranked risk report with ≤ 10 findings
...
Running `wrkr scan --diff` against a previously scanned org shows only: new tools added, tools removed, tools with changed permissions/configs, and new risk findings. No false positives from unchanged repos.

### AC7: The "Zero Egress" Audit

A network-isolated run (no outbound internet after install) completes successfully. Scan results are verified to exist only on local filesystem. No DNS lookups to non-GitHub domains during scan.
...
Running `wrkr manifest generate` against a repo with Claude Code, Cursor, and 2 MCP servers produces a `wrkr-manifest.yaml` that lists all discovered tools under `approved_tools` with their current configuration (versions, hooks, MCP servers, access scopes), sets `blocked_tools` to empty, and populates the `policy` section with sensible defaults (`require_pinned_versions: true`, `require_mcp_approval: true`). Running `wrkr scan` against the same repo after committing the generated manifest treats all listed tools as approved (trust deficit = 0 on the approved axis). Removing a tool from the manifest and re-scanning flags it as unapproved with elevated risk.
...
Proof Record Emitter:
  ...
  ├── Converts LifecycleTransition → proof.Record (type: approval)
...
// Detector is the interface that detection plugins implement.
...
### The `wrkr-manifest.yaml` (convention for repos)

This is the file that teams add to their repos to declare their approved AI tool configuration. Wrkr generates it, and then uses it as the baseline for drift detection.

```yaml
# wrkr-manifest.yaml — Approved AI tool configuration for this repo
# Generated by Wrkr. Reviewed and approved by: @sam (2026-09-15)
version: "1.0"
...
approved_tools:
  - type: claude-code
    ...
    hooks:
      - name: pre-commit-review
        source: github.com/acme-corp/shared-hooks
        pinned_to: "abc123f"
    mcp_servers:
      - name: docs-search
        endpoint: mcp://docs.internal
        access: read-only
        approved_by: "@maria"
        approved_date: "2026-09-10"
    allowed_tools: [file_edit, web_search]
    blocked_tools: [bash]  # no shell exec in this repo
...
| Free/paid line is undefined — risk of giving away too much or gating too early | Medium | High | v1 free: CLI scan, risk report, remediation PRs, proof records, evidence bundle, posture regression — everything a solo engineer or small team needs. Pro: dashboard (inventory browser, risk trends), scheduled scans with Slack/webhook, approval workflows, SIEM export, SSO/RBAC, multi-org support. The line is: **CLI is free, orchestration is Pro.** The scan itself never requires a license. Revenue comes from the management layer that Maria (CISO) needs, not the scanning layer that Sam (engineer) runs. Validate with design partners in weeks 9-12 — if the free tier is too rich, tighten; if it gates adoption, loosen. |
