...
- `review_lifecycle_state` is additive review-loop state metadata and may be one of `open`, `confirmed`, `declared_controlled`, `covered_by_imported_control`, `accepted_risk`, `not_applicable`, `false_positive`, `needs_runtime_evidence`, `expired`, or `reopened_by_drift`.
- `review_audit_context`, `resolved_visibility`, `resolved_appendix_refs`, `previous_review_lifecycle_state`, `reopen_state`, `reopen_reasons`, and `reopen_evidence_refs` keep resolved and reopened customer review decisions auditable in JSON, Markdown appendix handoff, and regress baselines.
