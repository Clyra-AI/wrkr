...
- Action Contract v3 interoperability fixtures are release-pinned: the manifest producer is the historical released `v1.14.0` tag (the first tag containing all nine current artifact bytes and the three validating schemas). Tagged releases must descend from that producer tag, and the conformance generator compares generated artifact, packet, and schema bytes with the claimed producer tag before packaging.
