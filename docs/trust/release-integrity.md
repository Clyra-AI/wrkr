...
- Action Contract v3 interoperability fixtures are release-pinned: the manifest producer is the released `v1.14.0` tag (the first tag containing all nine current artifact bytes and the three validating schemas), and the conformance generator fails closed for missing, `devel`, malformed, wrong-tag, or untagged producer metadata. The tag release workflow runs the exact fixture check before packaging.
