...
Expected JSON keys: `status`, `output_dir`, `frameworks`, `manifest_path`, `chain_path`, `framework_coverage`, additive `control_evidence`, additive `coverage_note`, `report_artifacts`, additive `source_privacy`, additive `runtime_evidence`, additive `agent_action_bom`, additive `next_steps`.
...
When `wrkr ingest` has written a managed runtime evidence sidecar next to the selected state file, the evidence bundle includes `runtime-evidence.json` and `runtime-evidence-correlation.json` without mutating scan state. Correlation rows include normalized runtime and external-control evidence classes, additive repo/service/workflow/environment/path/target fields, policy refs, proof refs, required checks, owner refs, unmatched reasons, and graph refs used to support Agent Action BOM coverage and runtime-proof joins.
