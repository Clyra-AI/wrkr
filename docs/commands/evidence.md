...
Evidence bundles also carry additive `composition_refs[]` when composed action paths exist. These rows contain stable join material such as `composition_id`, `resolution_key`, member `path_ids[]`, `workflow_chain_refs[]`, `proposed_action_contract_refs[]`, recommendation, evidence state, policy coverage, and Gait coverage summary without duplicating proof record payloads.
