...
When `--json` is set, Wrkr keeps stdout reserved for the final JSON payload and emits progress to stderr only. Interactive stderr targets can render the live bar through `--progress auto`, while non-interactive stderr targets keep the event-style contract by default. Event-mode progress includes retry, cooldown, resume, per-repo materialization completion/discovery, detector lifecycle detail, heartbeat updates, scan phase transitions, completion, and final footer lines. For hosted scans, `repo_materialize` means a repo job was dispatched to a worker and `repo_materialize_done` means that repo reached a success or failure result. For path scans, `repo_discovered` means a local repo root was selected for detector execution. `--json-path` writes the same bounded JSON summary payload to disk through the same streaming encoder path used for other large JSON artifacts, and `--state` remains the deterministic scan artifact path. Large scans can omit heavyweight top-level arrays from the stdout/`--json-path` summary while preserving fuller bounded detail in `--state`. Any requested `--json-path`, `--report-md-path`, or `--sarif-path` must be unique from one another and from scan-managed `--state` sibling artifacts.
...
`inventory.canonical_stores` is the additive per-scan reference store for mutable endpoint semantics, credential authority, and authority bindings. `agent_privilege_map[*]` and `action_paths[*]` now also carry additive `mutable_endpoint_semantic_refs`, `credential_authority_ref`, and `authority_binding_refs` so downstream graph/BOM/report consumers can join to the canonical store instead of re-deriving those facts. The bounded scan JSON preview and persisted saved state omit repeated embedded endpoint/authority payload clones when those refs are present; load-time hydrated internal views can resolve them back from `inventory.canonical_stores`. `action_paths[*]` also carries additive `occurrence_count` / `occurrence_refs` when identical path identities are grouped.
