...
`agent_privilege_map[*]`, `inventory.tools[*]`, and `action_paths[*]` also emit additive static endpoint classification via `mutable_endpoint_semantics[]` (`read`, `write`, `delete`, `deploy`, `refund`, `payment`, `user_admin`, `data_export`, `production_mutation`) with deterministic confidence, surface, operation, and evidence refs. These fields are declaration-only; they do not claim live reachability or runtime observation.
