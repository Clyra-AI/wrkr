...
When `--json` is set, Wrkr keeps stdout reserved for the final JSON payload and emits progress to stderr only. Interactive stderr targets can render the live bar through `--progress auto`, while non-interactive stderr targets keep the event-style contract by default. Event-mode progress includes retry, cooldown, resume, per-repo materialization completion/discovery, detector lifecycle detail, heartbeat updates, scan phase transitions, completion, and final footer lines. For hosted scans, `repo_materialize` means a repo job was dispatched to a worker and `repo_materialize_done` means that repo reached a success or failure result. For path scans, `repo_discovered` means a local repo root was selected for detector execution. `--json-path` writes the same bounded JSON summary payload to disk, and `--state` remains the full deterministic scan artifact path. Large scans can omit heavyweight top-level arrays from the stdout/`--json-path` summary while preserving the complete artifact in `--state`. Any requested `--json-path`, `--report-md-path`, or `--sarif-path` must be unique from one another and from scan-managed `--state` sibling artifacts.
