...
Runtime session and evidence-packet sidecars may also carry additive provider-neutral context fields such as `runtime_provider`, `runtime_host`, `runtime_kind`, `model_provider`, `model_version`, and `execution_environment` when the local export already has them. Missing values stay absent; conflicting values are carried forward as explicit unknown/contradictory context rather than last-write-wins certainty.
Retained-state posture is additive and fail-closed: use `state_retention_status`, `retained_state_types`, `state_location_refs`, and `state_digest_refs` for refs and digests only. Wrkr rejects raw prompt, tool-result, checkpoint, log, sandbox-file, or memory-content payloads in those fields.
...
- Runtime evidence can also strengthen `composed_action_paths[*].claim_state` in reports when every consequential transition has matching enforcement and required outcome/proof evidence. Declared policy or static reachability remains `declared_policy_only`, `partially_evidenced`, or `static_only` until the imported evidence supports a runtime-controlled claim.
- Decision traces, Agent Action BOM primary views, and evidence bundles carry explicit `composition_ids[]`, `proposed_action_contract_refs[]`, `workflow_chain_refs[]`, and `composition_refs[]` joins when available, so imported sidecars can correlate to composition evidence without matching prose or volatile `path_id` values alone.
