...
Agent Action BOM items also carry buyer-facing `queue`, `finding_visibility`, and `remediation` fields, plus additive `inventory_risk`, `risk_tier`, `attack_path_refs`, `source_finding_keys`, and optional `exclusion_reason`. When a top attack path does not map to any govern-first action path, Wrkr emits a deterministic exclusion item instead of silently dropping the path from the BOM.
