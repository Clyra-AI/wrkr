...
When `wrkr ingest` has written a managed runtime evidence sidecar next to the selected state file, `summary.runtime_evidence` and top-level `runtime_evidence` expose deterministic path/agent correlation metadata without mutating saved scan findings.
