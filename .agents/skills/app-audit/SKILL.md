---
name: app-audit
description: Run an evidence-based audit for Wrkr OSS project across product, architecture, DX, security, and GTM readiness; output a strict go/no-go report with P0-P3 blockers and launch risk by technology, messaging, and expectations.
disable-model-invocation: true
---

# Wrkr OSS Audit

Execute this workflow when asked to perform app review, release readiness, architecture clarity, or public-launch audit for Wrkr.
...
6. Audit docs for integration clarity:
   - where Wrkr sits in runtime path
   - what is customer code vs Wrkr vs tool/provider
...
- `wrkr scan --json` to capture environment diagnostics in machine-readable form.
- `wrkr evidence --frameworks eu-ai-act,soc2 --json` to inspect artifact envelope integrity and payload shape.
- `wrkr regress run --baseline <baseline-path> --json` to validate fail-closed policy behavior.
