...
- `wrkr verify --chain --json` when validating artifact integrity in a failing CI path.
- `wrkr regress run --baseline <baseline-path> --json` when policy-path checks are implicated.
