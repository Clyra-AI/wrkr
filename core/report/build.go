...
func BuildSummary(in BuildInput) (Summary, error) {
	...
	if len(riskReport.ComposedActionPaths) == 0 {
	...
	} else if riskReport.ComposedActionPathToControlFirst == nil && len(riskReport.ComposedActionPaths) > 0 {
		riskReport.ComposedActionPathToControlFirst = &risk.ComposedActionPathToControlFirst{
			Summary: risk.SummarizeComposedActionPaths(riskReport.ComposedActionPaths),
			Path:    riskReport.ComposedActionPaths[0],
		}
	}
...
func buildSections(
	...
	if sourcePrivacy != nil {
		...
		if len(sourcePrivacy.Warnings) > 0 {
			headlineFacts = append(headlineFacts, fmt.Sprintf("source_privacy warnings=%d details_redacted", len(sourcePrivacy.Warnings)))
		}
	}
...
func decorateControlBacklogFromActionPaths(backlog *controlbacklog.Backlog, paths []risk.ActionPath) *controlbacklog.Backlog {
	...
	for idx := range copyBacklog.Items {
		...
		copyBacklog.Items[idx].DelegationReadinessState = firstNonEmptyValue(strings.TrimSpace(path.DelegationReadinessState), strings.TrimSpace(item.DelegationReadinessState))
		copyBacklog.Items[idx].DelegationReadinessReasons = uniqueStrings(append(append([]string(nil), item.DelegationReadinessReasons...), path.DelegationReadinessReasons...))
		copyBacklog.Items[idx].RecommendedControl = firstNonEmptyValue(strings.TrimSpace(path.RecommendedControl), strings.TrimSpace(item.RecommendedControl))
		copyBacklog.Items[idx].RecommendedControlReasons = uniqueStrings(append(append([]string(nil), item.RecommendedControlReasons...), path.RecommendedControlReasons...))
		copyBacklog.Items[idx].RiskClassificationValidationReasons = uniqueStrings(append(append([]string(nil), item.RiskClassificationValidationReasons...), path.RiskClassificationValidationReasons...))
		copyBacklog.Items[idx].RiskClassificationValidationRefs = uniqueStrings(append(append([]string(nil), item.RiskClassificationValidationRefs...), path.RiskClassificationValidationRefs...))
...
func sanitizeActionSurfaceRegistryPublic(in []ActionSurfaceRegistryEntry) []ActionSurfaceRegistryEntry {
	...
	for _, item := range in {
		copyItem := item
		copyItem.RegistryID = redactValue("surface", copyItem.RegistryID, 8)
		copyItem.Org = redactValue("org", copyItem.Org, 6)
		copyItem.Repo = redactValue("repo", copyItem.Repo, 6)
		copyItem.ToolInstanceID = redactValue("instance", copyItem.ToolInstanceID, 8)
		copyItem.Location = redactValue("loc", copyItem.Location, 8)
		copyItem.Label = redactValue("label", copyItem.Label, 8)
		copyItem.Owner = redactValue("owner", copyItem.Owner, 8)
		copyItem.ConfigSource = redactValue("loc", copyItem.ConfigSource, 8)
		copyItem.PathIDs = redactStringSlice(copyItem.PathIDs, "path")
		copyItem.GraphRefs = AgentActionBOMGraphRefs{
			NodeIDs: redactStringSlice(copyItem.GraphRefs.NodeIDs, "node"),
			EdgeIDs: redactStringSlice(copyItem.GraphRefs.EdgeIDs, "edge"),
		}
		if copyItem.CredentialAuthority != nil {
			copyItem.CredentialAuthority = agginventory.CloneCredentialAuthority(copyItem.CredentialAuthority)
			copyItem.CredentialAuthority.ReasonCodes = redactStringSlice(copyItem.CredentialAuthority.ReasonCodes, "evidence")
...
func sanitizeMutableEndpointSemanticsPublic(in []agginventory.MutableEndpointSemantic) []agginventory.MutableEndpointSemantic {
	if len(in) == 0 {
		return nil
	}
...
func proposedActionContractRefMap(raw, sanitized []risk.ComposedActionPath) map[string]string {
	...
	out := map[string]string{}
	for idx := 0; idx < len(raw) && idx < len(sanitized); idx++ {
		rawRefs := raw[idx].ProposedActionContractRefs
		sanitizedRefs := sanitized[idx].ProposedActionContractRefs
		if len(sanitizedRefs) == 0 && sanitized[idx].ProposedActionContract != nil && strings.TrimSpace(sanitized[idx].ProposedActionContract.ContractID) != "" {
			sanitizedRefs = []string{strings.TrimSpace(sanitized[idx].ProposedActionContract.ContractID)}
		}
		if len(rawRefs) == 0 || len(sanitizedRefs) == 0 {
			continue
		}
		for _, rawRef := range rawRefs {
			if trimmed := strings.TrimSpace(rawRef); trimmed != "" {
				out[trimmed] = strings.TrimSpace(sanitizedRefs[0])
			}
		}
	}
	if len(out) == 0 {
