...
type AgentActionBOMSummary struct {
	...
	ProductionImpactInferredItems int                                  `json:"production_impact_inferred_items"`
...
func buildAgentActionBOM(summary Summary, findings []model.Finding) *AgentActionBOM {
	...
	for _, path := range summary.ActionPaths {
		...
		item := AgentActionBOMItem{
			...
			DelegationReadinessReasons:          append([]string(nil), path.DelegationReadinessReasons...),
			RecommendedControl:                  strings.TrimSpace(path.RecommendedControl),
			RecommendedControlReasons:           append([]string(nil), path.RecommendedControlReasons...),
			RiskClassificationValidationReasons: append([]string(nil), path.RiskClassificationValidationReasons...),
			RiskClassificationValidationRefs:    append([]string(nil), path.RiskClassificationValidationRefs...),
...
func summarizeAgentActionBOMItems(items []AgentActionBOMItem, paths []risk.ActionPath, report *scanquality.Report, regressSummary *RegressSummary) AgentActionBOMSummary {
	...
	for _, item := range items {
		...
		} else if item.ProductionImpactInferred {
			counts.ProductionImpactInferredItems++
		}
		...
		if item.RuntimeEvidenceAbsenceStatus == "" &&
			(item.PolicyStatus == risk.PolicyCoverageStatusRuntimeProven || item.RuntimeEvidenceState == risk.EvidenceStateVerified) {
...
func evaluateBOMEmptyState(projection risk.ActionPathSummary, counts AgentActionBOMSummary, scanCoverageReduced bool) (string, []string) {
	...
	for _, blocker := range []struct {
		...
		{counts.ProductionImpactInferredItems, "production_impact_inferred_paths_present"},
