...
func runScanWithContext(parentCtx context.Context, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	reportShareProfile := fs.String("report-share-profile", "", "scan summary share profile [internal|public|customer-redacted|design-partner|external-redacted|investor-safe]")
	...
	progressMode, progressModeErr := parseScanProgressMode(*progressModeRaw)
	if progressModeErr != nil {
		return emitError(stderr, jsonRequested || *jsonOut, "invalid_input", progressModeErr.Error(), exitInvalidInput)
	}
	...
	*githubToken = resolveScanGitHubToken(*githubToken, cfg)
	...
	if *quiet {
		progressMode = scanProgressModeNone
	}
	if !anyTargetUsesProgress(targets) {
		progressMode = scanProgressModeNone
	}
	progress := newScanProgressReporter(scanProgressReporterOptions{
		...
		StatusSink:        statusTracker.Progress,
		HeartbeatInterval: scanProgressHeartbeatInterval(stderr),
	})
	progress.Start(ctx)
	defer progress.Stop()
...
func emitScanRuntimeError(stderr io.Writer, jsonOut bool, err error) int {
	...
	case isUnsafeManagedArtifactPathError(err):
