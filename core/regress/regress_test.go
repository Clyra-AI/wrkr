...
func TestCompareFlagsDeprecatedToolReappearance(t *testing.T) {
	t.Parallel()
	...
	result := Compare(baseline, current)
	if !result.Drift {
		t.Fatal("expected drift for deprecated tool reappearance")
	}
	found := false
	for _, reason := range result.Reasons {
	...
	if !found {
		t.Fatalf("expected deprecated reappearance reason, got %v", result.Reasons)
	}
}
