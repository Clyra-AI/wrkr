...
- GitHub App installation inventory remains future/additive platform scope and is not part of the current default OSS scan path until a dedicated org-signal implementation lands.
...
- Support repository mode (`wrkr scan --repo <owner/repo>`), organization mode (`wrkr scan --org <org>`), local path mode (`wrkr scan --path <dir>`), and additive repeatable target-set mode (`wrkr scan --target <mode>:<value> ...`) for mixed hosted and local scans in one deterministic run.
...
- `wrkr scan --production-targets <path>` — apply deterministic customer production target rules and emit a privilege budget (`write_capable_tools`, `credential_access_tools`, `exec_capable_tools`, `production_write.status`, `production_write.source`, `production_write.count`) plus per-agent privilege map; built-in target packs remain `builtin_inferred` context and cannot publish `production_write` counts
