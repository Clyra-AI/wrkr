...
This is the file that teams add to their repos to declare their AI tool configuration and governance status. `wrkr manifest generate` produces an initial manifest with all discovered tools in `review_pending` status. After human review, tools are moved to `approved_tools` (via `wrkr identity approve` or manual edit). The committed manifest becomes the baseline for drift detection.

```yaml
# wrkr-manifest.yaml — AI tool configuration for this repo
# Generated by Wrkr. Review required before tools are approved.
version: "1.0"
repo: acme-corp/payments-service

# Tools that have been reviewed and approved by a named approver.
# Trust deficit = 0 for these tools.
approved_tools:
  - type: claude-code
    version: ">=1.2.0"
    approved_by: "@maria"
...
# Tools discovered by wrkr but not yet reviewed.
# Generated by `wrkr manifest generate`. Trust deficit remains elevated.
review_pending:
  - type: cursor
    status: under_review
    first_seen: "2026-09-15T10:30:00Z"
