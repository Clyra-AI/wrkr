...
func TestReportOverclaimAcceptance(t *testing.T) {
	...
	t.Run("control-evidence-internal", func(t *testing.T) {
		...
		assertAcceptanceArtifactsPassBuyerQA(t, scanPayload, reportPayload, map[string]string{
...
func assertAcceptanceArtifactsPassBuyerQA(t *testing.T, scanPayload, reportPayload map[string]any, extraTexts map[string]string) {
	t.Helper()
	...
	if err := reportcore.ValidateBuyerArtifactTexts(reportcore.BuyerArtifactQAInput{
		ActionPathTypes: acceptanceActionPathTypes(t, scanPayload),
		Texts:           texts,
	}); err != nil {
...
func acceptanceActionPathTypes(t *testing.T, scanPayload map[string]any) []string {
	t.Helper()
