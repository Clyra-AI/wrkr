...
require_pattern "${REPO_ROOT}/docs/commands/fix.md" "wrkr fix computes a deterministic remediation plan from existing scan state and emits plan metadata; preview mode does not mutate repository files\\." "fix command docs missing explicit side-effect contract sentence one"
require_pattern "${REPO_ROOT}/docs/commands/fix.md" "When --open-pr is set, wrkr fix publishes deterministic preview PRs for the target repo; add --apply to write supported repo files instead of preview artifacts only\\." "fix command docs missing explicit side-effect contract sentence two"
...
require_pattern "${REPO_ROOT}/CHANGELOG.md" "scripts/finalize_release_changelog\\.py --json" "CHANGELOG missing release changelog finalization command"
require_pattern "${REPO_ROOT}/CHANGELOG.md" "scripts/validate_release_changelog\\.py --release-version vX\\.Y\\.Z --json" "CHANGELOG missing release changelog validation command"
