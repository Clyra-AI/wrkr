...
- Agent Action BOM summary now also carries additive `primary_view` workflow-BOM fields for the default or explicit `--focus-path` selection: selected `path_id`, selection reason, path map, evidence/control posture, additive `risk_tier`, additive concise `recommended_next_actions[]`, additive `coverage_status`, and appendix refs.
